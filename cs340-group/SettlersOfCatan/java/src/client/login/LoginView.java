...
public class LoginView extends OverlayView implements ILoginView
{
    ...
    private LoginLeftPanel leftPanel=null;
...
//        this.add(buttonPanel, BorderLayout.SOUTH);
    ...
    public String getIPAddress(){
        if(leftPanel!=null)return leftPanel.txtIPAddress.getText();
        else return null;
    }

    @Override
    public String getPort(){
        if(leftPanel!=null)return leftPanel.txtPort.getText();
        else return null;
    }
    @Override
    public void setIPConnectedMessage(String message){
        if(leftPanel!=null)leftPanel.setIPAddressMessage(message);
    }
    @Override
    public void setPortConnectedMessage(String message){
        if(leftPanel!=null)leftPanel.setPortMessage(message);
    }

    private class LoginLeftPanel extends JPanel{
        JLabel lblTitle=null;
        JLabel lblIPAddress=null;
        JLabel currentIPAddress=null;
        JLabel currentPort=null;
        JLabel adviceField=null;
        JTextField txtIPAddress=null;
        JLabel lblPort=null;
        ...
        public void setIPAddressMessage(String message){currentIPAddress.setText(message);}
        public void setPortMessage(String message){currentPort.setText(message);}
        private void initComponents(){

            lblTitle = new JLabel("<html><body>Settlers<br>of Catan</body></html>");
            Font labelFont = lblTitle.getFont();
            labelFont = labelFont.deriveFont(labelFont.getStyle(), LABEL_TEXT_SIZE);
            lblTitle.setFont(labelFont);
            ...
            txtIPAddress = new JTextField(NUM_TXT_COLS);
            lblPort = new JLabel("Port");
            txtPort = new JTextField(NUM_TXT_COLS);


            btnConnect = new JButton("Connect");

            currentIPAddress=new JLabel("No Connection");
            currentPort=new JLabel("No Connection");
        }
        private void setTooltips(){
            String localIPMessage="";
            String localIP="";
            try{
                localIP=Inet4Address.getLocalHost().getHostAddress();
                localIPMessage=", which is probably "+localIP;
            }catch (Exception e){
                localIPMessage="";
                localIP="";
            }
            txtIPAddress.setToolTipText("If you are running a server enter \"localhost\" to use your IP"+localIPMessage);
            txtIPAddress.setText(localIP);
            txtPort.setToolTipText("Default port is probably 8081");
            txtPort.setText("8081");
            Debugger.LogMessage(txtIPAddress.getToolTipText());
            Debugger.LogMessage("Current IP="+ HTTPServerProxy.getInstance().getHost());
        }
        private void initLayout()
        {
            final int strutHeight=75;
            this.setLayout(new BoxLayout(this, BoxLayout.PAGE_AXIS));

            this.add(lblTitle);

            this.add(Box.createVerticalGlue());
            this.add(Box.createVerticalStrut(strutHeight));

            JPanel internalInputBox = new JPanel(new GridLayout(6, 1));
            //Change the font on the user entry labels.
            Font smallTextFont = lblIPAddress.getFont();
            smallTextFont = smallTextFont.deriveFont(smallTextFont.getStyle(), SMALL_LABEL_TEXT_SIZE);
            lblIPAddress.setFont(smallTextFont);
            txtIPAddress.setFont(smallTextFont);
            lblPort.setFont(smallTextFont);
            txtPort.setFont(smallTextFont);

            Font tinyTextFont=smallTextFont.deriveFont(smallTextFont.getStyle(),SMALL_LABEL_TEXT_SIZE);
            currentIPAddress.setFont(tinyTextFont);
            currentPort.setFont(tinyTextFont);

            internalInputBox.add(currentIPAddress);
            internalInputBox.add(currentPort);
            internalInputBox.add(lblIPAddress);
            internalInputBox.add(txtIPAddress);
            internalInputBox.add(lblPort);
            internalInputBox.add(txtPort);
            this.add(internalInputBox);

            this.add(Box.createVerticalGlue());
            this.add(Box.createVerticalStrut(strutHeight/2));

            Font btnFont = btnConnect.getFont();

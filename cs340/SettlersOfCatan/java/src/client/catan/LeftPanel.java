...
public class LeftPanel extends JPanel {
	...
	public LeftPanel(TitlePanel titlePanel, GameStatePanel gameStatePanel) {
		...
		historyController = new GameHistoryController(historyView, ModelReferenceFacade.getInstance());
		historyView.setController(historyController);
		
		chatView = new ChatView();
        chatController = new ChatController(chatView, ModelReferenceFacade.getInstance());
        chatView.setController(chatController);
		
		turnView = new TurnTrackerView(titlePanel, gameStatePanel);
		turnController = new TurnTrackerController(turnView, ModelReferenceFacade.getInstance());

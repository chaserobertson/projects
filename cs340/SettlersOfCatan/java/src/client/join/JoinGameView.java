	...
	private void initializeView()
	{
		...
		if (games != null && games.length > 0)
		{
			...
			for (GameInfo game : games)
			{
				...
				gamePanel.add(tmp3);
				JButton joinButton=new JButton("null");

				for(int j=0;j<game.getPlayers().size();++j){
					if (game.getPlayers().get(j).getId()==localPlayer.getId())
					{
						joinButton = new JButton("Re-Join");
						break;
					}
					else if (j >= 3 && game.getPlayers().size() >= 4)
					{
				...
				if(joinButton.getText().equals("null")){

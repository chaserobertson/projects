...
instantiate(X, Row, Col, Cub) :-
        (nonvar(X); var(X), digit(X), is_set(Row), is_set(Col), is_set(Cub)).

check(Board, RowNum, ColNum) :-
        nth0(RowNum, Board, RowList),
        nth0(ColNum, RowList, Num),
        columnAsList(Board, ColNum, ColList),
        cubeNum(RowNum, ColNum, CubeNum),
        getCube(Board, CubeNum, CubeList),
        instantiate(Num, RowList, ColList, CubeList).

solveRow(Board, RowNum) :-
        maplist(check(Board, RowNum), [0,1,2,3,4,5,6,7,8]),
        true.

solve(Board) :- %recurse through: rows, cubes(repeating in 3s), & columns/members
        maplist(solveRow(Board), [0,1,2,3,4,5,6,7,8]),
        true.

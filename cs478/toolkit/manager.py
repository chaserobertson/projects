...
from .multiple_perceptron import MultiplePerceptronLearner
from .decision_tree_learner import DecisionTreeLearner
from .knn_learner import InstanceBasedLearner
...
class MLSystemManager:
    ...
    def get_learner(self, model):
        ...
        modelmap = {
            ...
            "perceptron": MultiplePerceptronLearner(),
            "multiplePerceptron": MultiplePerceptronLearner(),
    ...
    def main(self):
        ...
        if eval_method == "training":
            ...
            features = Matrix(data, 0, 0, data.rows, data.cols-self.n_outputs)
            labels = Matrix(data, 0, data.cols-self.n_outputs, data.rows, self.n_outputs)
            confusion = Matrix()
            ...
            # original_acc = learner.measure_accuracy(train_features, train_labels)
            # print("Pre-reduction training set accuracy: {}".format(original_acc))
            # cur_acc = original_acc
            # removed = 0
            # while (original_acc + 0.02) >= cur_acc:
            #     new_features.shuffle(new_labels)
            #     new_features = Matrix(new_features, 0, 0, new_features.rows - 1, new_features.cols)
            #     new_labels = Matrix(new_labels, 0, 0, new_labels.rows - 1, new_labels.cols)
            #     learner.train(new_features, new_labels)
            #     cur_acc = learner.measure_accuracy(train_features, train_labels)
            #     removed += 1
            # print("Instances removed: %d of %d / %f" % (removed, train_features.rows, (removed / train_features.rows)))
            ...
            train_features = Matrix(data, 0, 0, train_size, data.cols-self.n_outputs)
            train_labels = Matrix(data, 0, data.cols-self.n_outputs, train_size, self.n_outputs)

            test_features = Matrix(data, train_size, 0, data.rows - train_size, data.cols-self.n_outputs)
            test_labels = Matrix(data, train_size, data.cols-self.n_outputs, data.rows - train_size, self.n_outputs)
            ...
            test_accuracy = learner.measure_accuracy(test_features, test_labels, confusion)
            print("Test set accuracy: {}".format(test_accuracy))
            # test_mse = learner.test_mse(test_features, test_labels)
            # print('Test set MSE: %f' % test_mse[0])
        ...
        elif eval_method == "unsupervised":

            data = Matrix(data, 0, 0, data.rows, data.cols)
            # learner.train(data, Matrix())
            my_learner = ClusterLearner()
            ks = [x for x in range(2, 11)]
            sils = []
            for k in ks:
                sils.append(my_learner.my_train(data, k))
            print(ks)
            print(sils)
            print(np.argmin(sils))
            print('Optimal number of clusters: %d' % ks[int(np.argmin(sils))])
    ...
    def parser(self):
        ...
        parser.add_argument('-L', required=True, choices=['baseline', 'perceptron', 'multiplePerceptron', 'backprop',
                                                          'neuralnet', 'decisiontree', 'knn', 'cluster'], help='Learning Algorithm')
        ...
        parser.add_argument('-E', metavar=('METHOD', 'args'), required=True, nargs='+',
                            help="Evaluation method (training | static <test_ARFF_file> | random <%%_for_training> | cross <num_folds> | unsupervised)")

...
class WireframeViewer(wf.WireframeGroup):
    ...
    def __init__(self, width, height, name="Wireframe Viewer"):
        ...
        self.diffuse_reflect = np.array([0.33, 0.33, 0.33])
        self.specular_reflect = np.array([0.33, 0.33, 0.33])
        self.gloss = 4
            ...
            if self.displayFaces:
                for (face, colour) in wireframe.sortedFaces():
                    ...
                    if towards_us > 0:
                        ...
                        #Use the Phong model
                        diffuse = (self.light_color * self.diffuse_reflect) * np.dot(normal, self.light_vector)
                        
                        reflect_vector = (2 * np.dot(self.light_vector, normal) * normal) - self.light_vector
                        specular = (self.light_color * self.specular_reflect) * \
                        			np.power(np.dot(self.view_vector, reflect_vector), self.gloss)
                        ...
                        if np.dot(normal, self.light_vector) > 0:
                        	light_total = ambient + (diffuse * colour) + (specular * colour)
    ...
    def keyEvent(self, key):
        ...
        # rotate about y axis
        if key == pygame.K_a:
        	matrix = np.array([[np.cos(rotation), 0, np.sin(rotation), 0],
        						[0, 1, 0, 0],
        						[-np.sin(rotation), 0, np.cos(rotation), 0],
        						[0, 0, 0, 1]])
        	self.light_vector = np.delete(np.dot(matrix, np.append(self.light_vector, 1)), 3).flatten()
        if key == pygame.K_d:
        	matrix = np.array([[np.cos(-rotation), 0, np.sin(-rotation), 0],
        						[0, 1, 0, 0],
        						[-np.sin(-rotation), 0, np.cos(-rotation), 0],
        ...
        # rotate about z axis
        if key == pygame.K_q:
        	matrix = np.array([[np.cos(-rotation), -np.sin(-rotation), 0, 0],
